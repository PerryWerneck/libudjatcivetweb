 ...
 */
 ...
 namespace Udjat {
	...
	HTTP::StatusCode CivetWeb::Connection::send(const char *name, time_t maxage, const MimeType mimetype = MimeType::none) noexcept {

		if(!(name && *name)) {
			return send(
				HTTP::Status{
					HTTP::SystemError,
					_("Empty filename on request")		
				},
				0,
				MimeType::html,
				false
		...
		auto mimetype = MimeTypeFactory(filename.c_str());
		...
		if(stat(filename.c_str(), &st) < 0) {
			...
			HTTP::Status status;
			status.assign(errno);
			error(status.code,filename.c_str());
		...
		if(!S_ISREG(st.st_mode)) {
			error(HTTP::SystemError,String{filename.c_str()," is not a regular file"}.c_str());
			return send(
				HTTP::Status{
					HTTP::SystemError,
					_("Invalid filename on request")		
				},
				0,
				MimeType::html,
				false
		...
		// TODO: Check for 'not-modified'

		//
		// It's a file, send it.
		//
		if(Logger::enabled(Logger::Debug)) {
			info(HTTP::Ok,String {"Sending static file '",filename,"'"}.c_str());
		}
		...
		response_header_add_authentication();
	...
	HTTP::StatusCode CivetWeb::Connection::send(const HTTP::StatusCode code, time_t maxage, const MimeType mimetype, const char *payload) noexcept {
